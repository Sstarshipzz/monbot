...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not access_manager.is_authorized(user.id):
        ...
        welcome_msg = await context.bot.send_message(
            chat_id=chat_id,
            text="🔒 Bienvenue ! Pour accéder au bot, veuillez entrer votre code d'accès. Si vous n'avez pas de code d'accès vous pouvez en demander un en écrivant à @ASwissPlug"
        )
